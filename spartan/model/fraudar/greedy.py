...
def logAllDegree(M):
    # Add row weights for anti-money laundering!
    # You can remove np.log2 for other use.
    (m, n) = M.shape
    colSums = M.sum(axis=0)
    rowSums = M.sum(axis=1)
    rowWeights = 1.0 / np.log2(np.squeeze(rowSums.A) + 5)
    colWeights = 1.0 / np.log2(np.squeeze(colSums.A) + 5)
    rowDiag = sparse.lil_matrix((m, m))
    rowDiag.setdiag(rowWeights)
    colDiag = sparse.lil_matrix((n, n))
    colDiag.setdiag(colWeights)
    for a,b in zip(M.nonzero()[0],M.nonzero()[1]):
        M[a,b] = np.log2(M[a,b])
    W = rowDiag * M * colDiag
    print("finished computing weight matrix")
    return greedyDecreasing(W)
...
def greedyDecreasing(M):
    # We'd better not utilize colweights to update the priority in anti-money laundering!
    (m, n) = M.shape
    Md = M.todok()
    Ml = M.tolil()
    ...
    while rowSet and colSet:
        ...
        if rowDelt <= colDelt:
            ...
            for j in Ml.rows[nextRow]:
                colTree.changeVal(j, -Ml[nextRow,j])
            rowSet -= {nextRow}
            ...
            for i in Mlt.rows[nextCol]:
                rowTree.changeVal(i, -Ml[i,nextCol])
            colSet -= {nextCol}
